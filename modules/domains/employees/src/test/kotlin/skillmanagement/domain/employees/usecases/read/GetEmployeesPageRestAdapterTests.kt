...
@WebMvcTest(GetEmployeesPageRestAdapter::class)
@Import(GetEmployeesPageRestAdapterTestsConfiguration::class)
@AutoConfigureRestDocs("build/generated-snippets/employees/get-page", uriPort = 80)
internal class GetEmployeesPageRestAdapterTests(
    @Autowired val mockMvc: MockMvc,
    @Autowired val getEmployeesPage: GetEmployeesPageFunction
) {

    @Test
    fun `responds with 200 Ok and empty list if there are no Employees`() {
                ...
                content {
                    ...
                    strictJson {
                        """
                        {
                          "_links": {
                            "self": {
                              "href": "http://localhost/api/employees?page=0&size=100"
                            }
                          },
                          "page": {
                            "size": 100,
        ...
        verify { getEmployeesPage(AllEmployeesQuery(Pagination.DEFAULT)) }
    }

    @Test
    fun `responds with 200 Ok and Employees if there are any`() {
        every { getEmployeesPage(any()) } returns pageOf(listOf(employee_jane_doe, employee_john_smith))
    ...
    fun `responds with 200 Ok and Employees on requested page`() {
        every { getEmployeesPage(any()) } returns pageOf(listOf(employee_john_doe), 1, 1, 3)
                ...
                content {
                    ...
                    strictJson {
                        """
                        {
                          ...
                          "_links": {
                            "self": {
                              "href": "http://localhost/api/employees?page=1&size=1"
                            },
                            "previousPage": {
                              "href": "http://localhost/api/employees?page=0&size=1"
                            },
                            "nextPage": {
                              "href": "http://localhost/api/employees?page=2&size=1"
                            }
                          },
                          "page": {
                            "size": 1,
        ...
        verify { getEmployeesPage(AllEmployeesQuery(Pagination(PageIndex(1), PageSize(1)))) }
    }
    ...
    fun `paging data is handed over correctly`() {
        ...
        mockMvc
            .get("/api/employees?page=2&size=42")
            .andExpect { status { isOk() } }

        verify { getEmployeesPage(AllEmployeesQuery(Pagination(PageIndex(2), PageSize(42)))) }
    }

}

private class GetEmployeesPageRestAdapterTestsConfiguration {
    @Bean
